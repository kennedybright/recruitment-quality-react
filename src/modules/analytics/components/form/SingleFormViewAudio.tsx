...
import { StyledInputField } from '../../../../modules/qaforms/styles'
...
const QAFormSingleView: FC<QAFormProps> = ({form}) => {
    ...
    const scoringDropdowns = getFieldsbyType(audioFormFields, "scoring_dropdown").filter(field => field.label !== 'disposition')
    const qaCheckboxes = getFieldsbyType(audioFormFields, "form_checkbox")
    const mcaCheckboxes = getFieldsbyType(audioFormFields, "scoring_checkbox")

    const { formattedDate, formattedTime } = formatDateTime(new Date(`${form.record_date}T${form.record_time}`))

    return (
        <FlexWrapper400800 column gap={aliasTokens.space450} className='single-qa-form-view'>
