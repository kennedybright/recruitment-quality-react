...
import { formatPickerAsDate } from '../../../lib/utils/shared'
import { useEffect, useState } from 'react'
import { useDataContext } from '../../../lib/context/static-data'
import { FlexSection } from '../../qaforms/qaforms-audio-smp/styles'

const SingleModeOptions = () => {
    const { recordNumber, setRecordNumber, setFilterError, filterError } = useEditContext()

    // Check if the entered record number is a valid number
    useEffect(() => {
        if (recordNumber.length > 0) setFilterError(!/^\d+/.test(recordNumber))
    }, [recordNumber])
...
const BulkModeOptions = () => {
    const { riList } = useDataContext()
    const { riID, setRI, setRecordDate, recordDate } = useEditContext()
...
const ChooseMethod = () => {
    ...
    const { auditTrackingList } = useDataContext()
    const modes = ['single', 'bulk']

    return (
        <Flex className='choose-method' column alignItems='center' justifyContent='center' gap={aliasTokens.space800}>
            <FlexSection className='body-header' column>
