...
import { FormChange, useEditContext } from '../edits/edits'
...
const FormChangesTable: FC<FormEditsTableProps> = ({changes, ...props}) => {
  // convert boolean values to "Yes" or "No" string values 
  const updatedChanges = changes.map((change) => {
    return {
      ...change,
      old_value: typeof change.old_value === "boolean" ? `${change.old_value}` : change.old_value,
      new_value: typeof change.new_value === "boolean" ? `${change.old_value}` : change.new_value
    }
  })
  ...
  return (
    <FlexFormChangesBody className='review-changes' column>
      ...
      {formChanges.length > 0 && (
        <FormChangesTable className='form-changes' changes={formChanges} />
      )}
    </FlexFormChangesBody>
  )
}
